...
% Settings
K_vec =[10]';                                                       % Nr UEs
D_vec = [20,40];                                                    % Nr directions for the probing
...
P_ue = 10^((26-30)/10);             % Transmit power at the UE = 26 dBm
sigma2n = 10^((-80-30)/10);         % Noise power

% frame
tau_c = 2000;                        % number of samples
L = 40;
...
for run_ind = 1:N_run    
    for ind_k = 1:length(K_vec)
        ...
        [MSE] = channel_estiamation_MSE(M,L,K,sigma2n,P_ue, G_0_los, Theta_prob_pow, Theta_opt_pow, h_0_los, eta);
