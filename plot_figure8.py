...
if __name__ == '__main__':
    ...
    # Average configs
    gen_reflection_configs = gen_reflection_configs.mean(axis=-1)
    pow_reflection_configs = pow_reflection_configs.mean(axis=-1)
    sig_reflection_configs = sig_reflection_configs.mean(axis=-1)
    ...
    #pow_abs = 10 * np.log10(np.abs(pow_inner)**2)
    pow_abs = (np.abs(pow_inner)**2)
    angles_deg = np.rad2deg(angles)

    # Save the array to a text file
    pow_res = np.vstack((angles_deg, pow_abs)).T
    np.savetxt('pow_res.txt', pow_res, fmt='%.4f', comments='')
