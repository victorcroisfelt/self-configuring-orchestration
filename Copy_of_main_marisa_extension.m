...
SINR_bound_sig = zeros(K, numel(C_vec), numel(false_alarm_prob_vec), N_setup, N_channel_realizations);
SE_bound_sig = zeros(K, numel(C_vec), numel(false_alarm_prob_vec), N_setup, N_channel_realizations);

SINR_bound_pow = zeros(K, numel(C_vec), numel(false_alarm_prob_vec), N_setup, N_channel_realizations);
SE_bound_pow = zeros(K, numel(C_vec), numel(false_alarm_prob_vec), N_setup, N_channel_realizations);
...
for ind_setup = 1:N_setup
    ...
    for ind_ch =1:N_channel_realizations
        ...
        par_hat_detected_ue_sig = zeros(numel(C_vec), numel(false_alarm_prob_vec));
        par_hat_detected_ue_pow = zeros(numel(C_vec), numel(false_alarm_prob_vec));
        
        par_th_detected_ue_sig = zeros(numel(C_vec), numel(false_alarm_prob_vec));
        par_th_detected_ue_pow = zeros(numel(C_vec), numel(false_alarm_prob_vec));
        
        par_MSE_cha_est_sig = zeros(numel(C_vec), numel(false_alarm_prob_vec));
        par_MSE_cha_est_pow = zeros(numel(C_vec), numel(false_alarm_prob_vec));
        
        par_SINR_est_sig = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        par_SE_est_sig = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        
        par_SINR_est_pow = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        par_SE_est_pow = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        
        par_SINR_bound_sig = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        par_SE_bound_sig = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        
        par_SINR_bound_pow = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        par_SE_bound_pow = zeros(K, numel(C_vec), numel(false_alarm_prob_vec));
        ...
        for ind_d = 1:length(C_vec)
            ...
            for ind_prob = 1:numel(false_alarm_prob_vec)
                ...
                hat_detected_ue_sig(ind_d,ind_prob) = hat_det_rate_sig;
                hat_detected_ue_pow(ind_d,ind_prob) = hat_det_rate_pow;
                
                th_detected_ue_sig(ind_d,ind_prob) = th_det_rate_sig;
                th_detected_ue_pow(ind_d,ind_prob) = th_det_rate_pow;

                MSE_cha_est_sig(ind_d,ind_prob) = MSE_sig;
                MSE_cha_est_pow(ind_d,ind_prob) = MSE_pow;
                
                SINR_est_sig(:,ind_d,ind_prob) = SINR_sig;
                SINR_est_pow(:,ind_d,ind_prob) = SINR_pow;
                
                SE_est_sig(:,ind_d,ind_prob) = SE_sig;
                SE_est_pow(:,ind_d,ind_prob) = SE_pow;

                SINR_bound_sig(:,ind_d,ind_prob,ind_setup,ind_ch) = SINR_b_sig;
                SINR_bound_pow(:,ind_d,ind_prob,ind_setup,ind_ch) = SINR_b_pow;
                
                SE_bound_sig(:,ind_d,ind_prob,ind_setup,ind_ch) = SE_b_sig;
                SE_bound_pow(:,ind_d,ind_prob,ind_setup,ind_ch) = SE_b_pow;
